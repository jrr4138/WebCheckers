...
<body>
  <div class="page">
    ...
    <div class="body">
      <div><p>Number of spectators ${spectators}</p></div>
     
      <#if "${viewMode}" == "SPECTATOR">
        <#if "${spectating.name}" == "${redPlayer.name}"> 
          <a href="/spectate?name=${whitePlayer.name}&opt="y"">switch gameview</a>
        </#if>
        <#if "${spectating.name}" == "${whitePlayer.name}"> 
          <a href="/spectate?name=${redPlayer.name}&opt="y"">switch gameview</a>
        </#if>
      </#if>
