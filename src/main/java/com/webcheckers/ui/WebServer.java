...
public class WebServer {
  ...
  public void initialize() {
    ...
    //Handles the data received from the sign-in page
    post(SIGN_IN_URL, new PostSignInRoute(templateEngine));

    //Handles the data received from the sign-out page
    ...
    // Resigns the player from the game and ends the game
    post(RESIGN_URL, new PostResignRoute(gson));

    // Allows the player to spectate a game
    get(SPECTATE_URL, new GetSpectatingRoute(templateEngine));

     // Get spectate-stop-watching request
    get(SPECTATE_STOP_WATCH_URL, new GetSpectatorStopWatchingRoute(templateEngine));

    // Post spectator-check-turn request
    post(SPECTATE_CHECK_TURN_ROUTE, new PostSpectatorCheckTurnRoute(templateEngine, gson));

    // Get the rules for american checkers
